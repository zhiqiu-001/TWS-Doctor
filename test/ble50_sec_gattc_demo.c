...
* Modified to support UART command control: scan on demand, connect to selected device.
*
****************************************************************************/
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:
        ...
        printf("[CONNECTED] conn_id=%d addr="ESP_BD_ADDR_STR"\n",
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        printf("[NOTIFY] handle=0x%02x value=", p_data->notify.handle);
    ...
    case ESP_GATTC_DISCONNECT_EVT:
        ...
        printf("[DISCONNECTED] addr="ESP_BD_ADDR_STR" reason=0x%02x\n",
               ESP_BD_ADDR_HEX(p_data->disconnect.remote_bda), p_data->disconnect.reason);
        is_connected = false;
        get_service = false;
        break;
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_EXT_ADV_REPORT_EVT: {
        ...
        // Report every discovered device via UART
        char name_buf[64] = {0};
        if (adv_name && adv_name_len > 0) {
            int copy_len = adv_name_len < (int)sizeof(name_buf) - 1 ? adv_name_len : (int)sizeof(name_buf) - 1;
            strncpy(name_buf, (char *)adv_name, copy_len);
            name_buf[copy_len] = '\0';
        }
        printf("[DEVICE] addr_type=%d addr="ESP_BD_ADDR_STR" rssi=%d name=%s\n",
    ...
    case ESP_GAP_BLE_EXT_SCAN_STOP_COMPLETE_EVT:
        if (param->ext_scan_stop.status != ESP_BT_STATUS_SUCCESS){
            ...
            printf("[ERROR] Scan stop failed\n");
            break;
        }
        ESP_LOGI(GATTC_TAG, "Scanning stop successfully");
        printf("[SCAN] Scanning stopped\n");
        break;
...
// Target device info for connection (set via UART command)
...
{
    // Parse MAC address like XX:XX:XX:XX:XX:XX
    int values[6];
...
static void connect_to_device(uint8_t addr_type, esp_bd_addr_t addr)
{
    if (is_connected) {
        printf("[ERROR] Already connected to a device, disconnect first\n");
        return;
    }
...
static void handle_uart_command(const char *cmd)
{
    ...
    if (strcmp(cmd, "AT+SCAN") == 0) {
    ...
    } else if (strcmp(cmd, "AT+SCANSTOP") == 0) {
        esp_ble_gap_stop_ext_scan();
    } else if (strncmp(cmd, "AT+CONNECT=", 11) == 0) {
        // Format: AT+CONNECT=addr_type,XX:XX:XX:XX:XX:XX
        const char *params = cmd + 11;
        int addr_type = 0;
        char addr_str[18] = {0};
        if (sscanf(params, "%d,%17s", &addr_type, addr_str) == 2) {
            esp_bd_addr_t addr;
            if (parse_bd_addr(addr_str, addr)) {
                printf("[CONNECTING] addr_type=%d addr=%s\n", addr_type, addr_str);
                connect_to_device((uint8_t)addr_type, addr);
            } else {
                printf("[ERROR] Invalid address format: %s\n", addr_str);
            }
        } else {
            printf("[ERROR] Invalid CONNECT params: %s\n", params);
        }
    } else if (strcmp(cmd, "AT+DISCONNECT") == 0) {
        if (!is_connected) {
        ...
        printf("[HELP]   AT+SCAN              - Start BLE scanning\n");
        printf("[HELP]   AT+SCANSTOP          - Stop BLE scanning\n");
        printf("[HELP]   AT+CONNECT=type,addr - Connect to device (e.g. AT+CONNECT=0,7c:df:a1:40:01:dd)\n");
        printf("[HELP]   AT+DISCONNECT        - Disconnect current device\n");
        printf("[HELP]   AT+HELP              - Show this help\n");
    } else {
...
static void uart_command_task(void *pvParameters)
{
    ...
    while (1) {
        int len = uart_read_bytes(UART_PORT, (uint8_t *)cmd_buf + pos, 1, pdMS_TO_TICKS(50));
        if (len > 0) {
            char c = cmd_buf[pos];
            if (c == '\n' || c == '\r') {
                cmd_buf[pos] = '\0';
...
void app_main(void)
{
    ...
    printf("[INIT] BLE security client initialized. Type AT+HELP for commands.\n");
}
